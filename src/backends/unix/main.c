...
main(int argc, char **argv)
{
	...
	/* Prevent running Runblader as root. Only very mad
	   minded or stupid people even think about it. :) */
	if (getuid() == 0)
	{
		printf("Runblader shouldn't be run as root! Backing out to save your ass. If\n");
		printf("you really know what you're doing, edit src/unix/main.c and remove\n");
		printf("this check. But don't complain if Runblader eats your dog afterwards!\n");
	...
	if (getuid() != geteuid())
	{
		...
		printf("complain if Runblader eats your dog afterwards!\n");
