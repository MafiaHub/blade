...
void actor_use(edict_t *self, edict_t *other, edict_t *activator)
{
	...
	if (other && other->client)
	{
		...
		{
			self->followentity = NULL;
			self->goalentity = self->movetarget = self->oldmovetarget;
		}
		else
		{
			self->oldmovetarget = self->movetarget;
			self->movetarget = NULL;
			self->followentity = other;
			HuntTarget(self);
			self->stopping_dist = 20;
			self->target = NULL;
			return;
		}
	}
